...
<head>
    ...
    $id_after=isset($_GET['id_after']) ? $_GET['id_after'] : 0;
    ...
    if( empty($id_after) ){
        ...
        if(empty($catid)){
//    echo "catid为0";
            $data = $db -> query("SELECT n.id, n.title, n.inputtime,d.pictureurls, d.content FROM `zmit_picture` n LEFT JOIN `zmit_picture_data` d ON n.id = d.id  WHERE n.id <$id_after order by n.id desc limit 0,1") -> fetch();
          $isempty=$data['id'];
            if(empty($isempty)){
        ...
        if(!empty($catid)){
            ...
            $data = $db -> query("SELECT n.id, n.title, n.inputtime,d.pictureurls, d.content FROM `zmit_picture` n LEFT JOIN `zmit_picture_data` d ON n.id = d.id  WHERE n.id <$id_after and n.catid=$catid order by n.id desc limit 0,1") -> fetch();
            $isempty=$data['id'];
            if(empty($isempty)){
                echo "<script> alert('没有了！');window.history.back();</script>";
                return false;
