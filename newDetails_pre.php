...
<head>
    ...
    <?php
    require 'config.php';
    $id_pre=isset($_GET['id_pre']) ? $_GET['id_pre'] : 0;
    ...
    if( empty($id_pre) ){
        ...
        if(empty($catid)){
//    echo "catid为0";
            $data = $db -> query("SELECT n.id, n.title, n.inputtime, d.content FROM `zmit_news` n LEFT JOIN `zmit_news_data` d ON n.id = d.id WHERE n.id >$id_pre order by n.listorder desc , n.id asc limit 0,1") -> fetch();
            $isempty=$data['id'];
            if(empty($isempty)){
        ...
        if(!empty($catid)){
            $data = $db -> query("SELECT n.id, n.title, n.inputtime, d.content FROM `zmit_news` n LEFT JOIN `zmit_news_data` d ON n.id = d.id  WHERE n.id >$id_pre and n.catid=$catid order by n.listorder desc ,n.id asc limit 0,1") -> fetch();
            $isempty=$data['id'];
            if(empty($isempty)){
                echo "<script> alert('没有了！');window.history.back();</script>";
                return false;
