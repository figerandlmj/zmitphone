...
<head>
    ...
    $id4 = isset($_GET['id4']) ? $_GET['id4'] : 0;
    $id5 = isset($_GET['id5']) ? $_GET['id5'] : 0;
    if( empty($id1) ){
        echo "<script> alert('非法访问！');parent.location.href='index.php'; </script>";
    }
    else{
    $data['solve'] = $db -> query("SELECT `title`, `content` FROM `zmit_page` WHERE catid = $id1 or catid = $id2 or catid = $id3 or catid = $id4 or catid = $id5") -> fetchAll();
    }
    ?>
