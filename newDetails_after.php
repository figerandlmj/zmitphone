...
<head>
    ...
    if( empty($id_after) ){
        ...
        if(empty($catid)){
            $data = $db -> query("SELECT n.id, n.title, n.inputtime, d.content FROM `zmit_news` n LEFT JOIN `zmit_news_data` d ON n.id = d.id  WHERE n.id <$id_after order by n.listorder,n.id desc limit 0,1") -> fetch();
            $isempty=$data['id'];
            if(empty($isempty)){
        ...
        if(!empty($catid)){
            $data = $db -> query("SELECT n.id, n.title, n.inputtime, d.content FROM `zmit_news` n LEFT JOIN `zmit_news_data` d ON n.id = d.id  WHERE n.id <$id_after and n.catid=$catid order by n.listorder,n.id desc limit 0,1") -> fetch();
            $isempty=$data['id'];
            if(empty($isempty)){
                echo "<script> alert('没有了！');window.history.back();</script>";
                return false;
